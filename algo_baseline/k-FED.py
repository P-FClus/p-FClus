...
class kFED:
    ...
    #added by us to calculate the clustering cost of kfed
    def clust_cost(self,data,centers ,sparse=False, remean=False):
        ...
        for device in data:
            ...
            for clusterid in np.unique(clustering):
                ...
                dist = self.distance_to_set(points, centers, sparse=sparse)
                cost +=sum(dist)
            costs.append(cost/device.shape[0])
        return costs
...
for seed in seeds:
    ...
    for algo_class in algos:
        ...
        #added to store kfed clust cost 
        kfed_clust_cost = algo.clust_cost(data,centers) #clustering cost for each client
        
    mean_kFED = sum(clust_cost)/total_devices
    clustering_cost_per_run.append(mean_kFED)
    max_kFED = max(clust_cost)
    max_cost_per_run.append(max_kFED)

    #std_dev
    variance_kFED = 0
    for i in range (total_devices):
        variance_kFED += (clust_cost[i] - mean_kFED)**2
        
    variance_kFED /= total_devices

    std_dev_kFED = variance_kFED**0.5

    stdev_per_run.append(std_dev_kFED)

# mean and std dev of mean
num_runs = len(seeds)
avg_mean_kFED = sum(clustering_cost_per_run)/num_runs

avg_mean_variance_kFED = 0

for i in range (num_runs):
    avg_mean_variance_kFED += (clustering_cost_per_run[i] - avg_mean_kFED)**2

avg_mean_variance_kFED /= num_runs

stdev_mean_kFED = avg_mean_variance_kFED**0.5

# mean and std dev of std dev
avg_stdev_kFED = sum(stdev_per_run)/num_runs

avg_stdev_variance_kFED = 0
for i in range (num_runs):
    avg_stdev_variance_kFED += (stdev_per_run[i] - avg_stdev_kFED)**2

avg_stdev_variance_kFED /= num_runs

stdev_std_dev_kFED = avg_stdev_variance_kFED**0.5

# mean and std dev of max_kFED
avg_max_kFED = sum(max_cost_per_run)/num_runs

avg_max_variance_kFED = 0
for i in range (num_runs):
    avg_max_variance_kFED += (max_cost_per_run[i] - avg_max_kFED)**2

avg_max_variance_kFED /= num_runs

stdev_max_kFED = avg_max_variance_kFED**0.5

info_kFED=[]
info_kFED.append(avg_mean_kFED)
info_kFED.append(stdev_mean_kFED)
info_kFED.append(avg_stdev_kFED)
info_kFED.append(stdev_std_dev_kFED)
info_kFED.append(avg_max_kFED)
info_kFED.append(stdev_max_kFED)

info = []
info.append(info_kFED)

# Define the filename for the CSV file
csv_filename = 'result_kFED.csv'
