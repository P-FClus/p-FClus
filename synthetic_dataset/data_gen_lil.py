...
for _ in range(1):
    ...
    for _ in range(1):
        ...
        for j in range(total_clients):
            curr = [] # stores newly generated datapoint
            ...
            for h in range(homo):
                i = gauss[h]
                total_points = points_in_client1+j*additional_points_per_cluster # total points in a client is calculated
                from_ith_cluster_to_jth_client = math.ceil(total_points/homo)  # number of data from one cluster to a client
                for n in range(from_ith_cluster_to_jth_client):
                    if i==0 or i==1:
                        x_val,y_val = gauss_2d(10*i,2)
                    elif i==2 or i==3:
                        x_val,y_val = gauss_2d(10*(i+1),2)
                    elif i==4 or i==5:
                        x_val,y_val = gauss_2d(10*(i+2),2)
                    elif i==6 or i==7:
                        x_val,y_val = gauss_2d(10*(i+3),2)
                    elif i==8 or i==9:
                        x_val,y_val = gauss_2d(10*(i+4),2)
                    elif i==10 or i==11:
                        x_val,y_val = gauss_2d(10*(i+5),2)
                    elif i==12 or i==13:
                        x_val,y_val = gauss_2d(10*(i+6),2)
                    elif i==14 or i==15:
                        x_val,y_val = gauss_2d(10*(i+7),2)
                    elif i==16 or i==17:
                        x_val,y_val = gauss_2d(10*(i+8),2)
                    elif i==18 or i==19:
                        x_val,y_val = gauss_2d(10*(i+9),2)
...
# File name for the CSV file
        file_name = f"k_{total_clusters}_c_{total_clients}_h_{homo}_p_{points_in_client1}_a_{additional_points_per_cluster}_liloverlap_big_data.csv"
